...
#         sys.stdout.write('\r' + spinner[idx % len(spinner)] + ' ' + run.status)
#         sys.stdout.flush()
#         # time.sleep(1)
#         idx += 1
...
def convert_pdf_to_text(file_name):
    """
    Converts a PDF file to text by extracting text from each page using OCR (Optical Character Recognition).

    This function detects the operating system and sets the appropriate path to the Poppler utility, 
    which is required for converting PDF pages to images. It then uses the pytesseract library to 
    extract text from each image.

    Parameters:
    file_name (str): The path to the PDF file to be converted.

    Returns:
    str: The extracted text from the PDF, with each page's text separated by a header indicating the page number.
